...
function CommentCard({ img, stars, userName, description, coffeeName, date }: CommentCardProps) {
  return (
    <div className="w-[379px] max-[1380px]:w-[350px] max-[1300px]:w-[330px] max-[400px]:w-[300px] bg-ReviewCardBg drop-shadow-custom over p-4 rounded-lg shadow-md relative flex flex-col items-center">
      <img
        src={img}
      ...
      <p className="text-revieWCardDesc font-bold w-[293px] text-center leading-[150%]">{description}</p>
      <div className="mt-[22px] text-center">
        <h3 className="font-bold text-base text-semiGreen leading-[150%]">{coffeeName}</h3>
        <p className="text-revieWCardDesc text-sm font-bold leading-[150%]">{date}</p>
      </div>
    </div>
  );
}
